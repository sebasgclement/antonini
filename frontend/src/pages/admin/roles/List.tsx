...
export default function RolesList() {
  ...
  const { items, setItems, loading, error, page, setPage, totalPages, search, setSearch, refetch } =
  ...
  return (
    <div className="vstack" style={{ gap: 12 }}>
      <div className="hstack" style={{ justifyContent: 'space-between' }}>
        ...
        <Link className="enlace" to="/admin/roles/crear">+ Nuevo</Link>
      </div>
      ...
      <div className="card" style={{ overflowX: 'auto' }}>
        {loading ? (
          ...
          <div style={{ padding: 16, color: 'var(--color-muted)' }}>No hay roles para mostrar.</div>
        ) : (
                ...
                <tr key={r.id} style={{ borderTop: '1px solid #1f2430' }}>
                  ...
                  <td style={{ padding: 8 }}>
                    <div className="hstack" style={{ justifyContent: 'flex-end', gap: 8 }}>
                      <Button onClick={() => nav(`/admin/roles/${r.id}/editar`)}>Editar</Button>
      ...
      <Confirm
        open={!!toDelete}
        title="Eliminar rol"
        message={<>Vas a eliminar <b>{toDelete?.name}</b>. Esta acción no se puede deshacer.</>}
