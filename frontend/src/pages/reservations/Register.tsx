...
export default function RegisterReservation() {
  ...
  const [price, setPrice] = useState<number | "">(""); // 💰 precio de venta
  const [deposit, setDeposit] = useState<number | "">(""); // 💵 seña
  const [creditBank, setCreditBank] = useState<number | "">(""); // 🏦 crédito bancario
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Si venimos redirigidos con un vehículo, precargar su precio
        if (vehicleIdParam) {
          const v = data.vehicles?.find(
            (x: Vehicle) => x.id === parseInt(vehicleIdParam)
          );
  ...
  // Cuando cambia el vehículo entregado → actualizar valor
  useEffect(() => {
    if (!tradeInVehicleId) {
      setTradeInValue("");
      return;
  ...
  useEffect(() => {
    ...
    const credit = Number(creditBank) || 0;
    const trade = hasTradeIn ? Number(tradeInValue) || 0 : 0;
    setBalance(total - paid - credit - trade);
  }, [price, deposit, creditBank, tradeInValue, hasTradeIn]);
      ...
      <form onSubmit={onSubmit} className="vstack" style={{ gap: 16 }}>
        ...
        <div className="card vstack" style={{ gap: 16 }}>
          ...
          <select
            value={vehicleId}
            onChange={(e) =>
              setVehicleId(parseInt(e.currentTarget.value) || "")
            }
            required
          >
        ...
        <div className="card vstack" style={{ gap: 16 }}>
          ...
          <select
            value={customerId}
            onChange={(e) =>
              setCustomerId(parseInt(e.currentTarget.value) || "")
            }
            required
          >
            <option value="">Seleccionar…</option>
        ...
        <div className="card vstack" style={{ gap: 16 }}>
          ...
          <div className="form-row">
            <Input
              ...
              type="number"
              value={creditBank as any}
              onChange={(e) =>
                setCreditBank(parseFloat(e.currentTarget.value) || "")
              }
            />
          </div>
          ...
          {hasTradeIn && (
            <div className="vstack" style={{ gap: 12 }}>
              ...
              <select
                ...
                onChange={(e) =>
                  setTradeInVehicleId(parseInt(e.currentTarget.value) || "")
                }
                required={hasTradeIn}
              >
                <option value="">Seleccionar…</option>
