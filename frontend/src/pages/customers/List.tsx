...
export default function CustomersList() {
  ...
  const { items, loading, error, page, setPage, totalPages, search, setSearch } =
    usePagedList<Customer>('/api/customers')
  ...
  const onDelete = async () => {
    if (!toDelete) return
    try {
      await api.delete(`/api/customers/${toDelete.id}`)
      setToast('Cliente eliminado')
      // recargar página actual (muy simple: volver a setPage para re-fetch)
      setPage(page) 
    } catch (e: any) {
    ...
    <div className="vstack" style={{ gap: 12 }}>
      ...
      <div className="card hstack" style={{ justifyContent: 'space-between' }}>
        <input
          ...
          style={{
            background: '#0c0f14', color: 'var(--color-text)', border: '1px solid #252b37',
            ...
            <tbody>
              {rows.map(c => (
                <tr key={c.id} style={{ borderTop: '1px solid #1f2430' }}>
                  ...
                  <td style={{ padding: 8 }}>{[c.doc_type, c.doc_number].filter(Boolean).join(' ') || '—'}</td>
                  <td style={{ padding: 8 }}>{c.email || '—'}</td>
                  <td style={{ padding: 8 }}>{c.phone || '—'}</td>
                  <td style={{ padding: 8 }}>
                    <div className="hstack" style={{ justifyContent: 'flex-end', gap: 8 }}>
                      <Button onClick={() => nav(`/clientes/${c.id}/editar`)}>Editar</Button>
      ...
      <Confirm
        ...
        message={<>Vas a eliminar <b>{displayCustomerName(toDelete || { id: 0 } as Customer)}</b>. Esta acción no se puede deshacer.</>}
