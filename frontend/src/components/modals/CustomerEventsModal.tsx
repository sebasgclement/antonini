...
export default function CustomerEventsModal({ customer, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    api.get(`/customers/${customer.id}/events`)
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" style={{maxWidth: 500, height: '80vh', display: 'flex', flexDirection: 'column'}} onClick={e => e.stopPropagation()}>
        
        {/* Header Modal */}
        <div className="hstack" style={{justifyContent: 'space-between', alignItems: 'center', marginBottom: 16}}>
            <h3 style={{margin:0}}>Agenda: {customer.first_name} {customer.last_name}</h3>
            <button onClick={onClose} style={{background:'none', border:'none', fontSize: '1.2rem', cursor:'pointer'}}>✕</button>
        </div>

        {/* Formulario de carga rápida */}
        <form onSubmit={handleSubmit} style={{background: 'var(--hover-bg)', padding: 12, borderRadius: 8, marginBottom: 16}}>
            ...
            <div className="hstack" style={{gap: 8}}>
                <input 
                    ...
                    style={{flex: 1}}
                />
                <Button type="submit" style={{padding: '6px 12px'}}>Agendar</Button>
            </div>
        </form>
        ...
        <div style={{flex: 1, overflowY: 'auto', paddingRight: 4}}>
            {loading ? <p style={{textAlign:'center'}}>Cargando historial...</p> : (
                <div className="vstack" style={{gap: 12}}>
                    {events.length === 0 && <p style={{color: 'var(--color-muted)', textAlign: 'center'}}>Sin eventos registrados.</p>}
